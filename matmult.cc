 ...
 */
void computeCol(const Matrix<double>& a, const Matrix<double>& b, size_t col, Matrix<double>& result)
{
  assert(a.numCols() == b.numRows());
  for (size_t r = 0; r < a.numRows(); r++) {
...
// }
//
Matrix<double> mult5(const Matrix<double>& a, const Matrix<double>& b) {
  assert(a.numCols() == b.numRows());
  Matrix<double> result(a.numRows(), b.numCols());
  {
    ThreadPool pool(4);
